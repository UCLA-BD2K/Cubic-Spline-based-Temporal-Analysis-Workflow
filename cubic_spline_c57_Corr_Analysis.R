...
setwd("/Users/howardchoi/Desktop/Gias_cys/");
...
dta_ftr =
  ...
  # select(protein_ptm = cys, control_xic = Intensity.Lj, iso_xic = Intensity.Hj, 
  #        iso_over_control = Ratio.H.L.normalizedj, sample = Sample) %>% 
  mutate(sample = sample %>% str_replace("7_3_2", "7_1")) %>% 
  separate(sample, into = c("timepoint", "replicate"), sep = "_");

ptm_ftr = 
  dta_ftr %>% 
  # gather(key = "treatment", value = "occupancy", control_xic:iso_xic) %>% 
...
ptm_ftr = 
  ...
  #filter(num_more_than_two >= 4 & num_more_than_one >= 6);
...
#####################################################################################################
### Data De-noising? or Modeling? via cubic spline
...
#####################################################################################################
# sep_num = 50;
# seq_range = seq(0, 14, length.out = sep_num);
...
######### testor ######################################################
plot(dta_modeling$r_squared, dta_modeling$p_value)

ind = 1
ind = ind + 1
# aPro = dta_ftr$protein_ptm %>% unique() %>% .[ind];
# aPro = dta_modeling %>% filter(r_squared> 0.0  & r_squared < 0.05) %>% unique() %>% .$protein_ptm %>% .[ind];
aPro = 
  dta_modeling %>% 
  filter(p_value > 0.7) %>% 
  .$protein_ptm %>% 
...
dots= 
  dta_ftr %>% 
  filter(protein_ptm == aPro)

mlines = 
dta_modeling %>% 
  gather(key = times, value = mods, SP_1:SP_14) %>% 
  filter(protein_ptm == aPro);

plot(as.numeric(dots$timepoint), dots$iso_over_control, ylim = c(0,2))
lines(seq_range,
      mlines$mods)
...
dta_ftr %>% 
  mutate(timepoint = parse_double(timepoint)) %>% 
  filter(protein_ptm == "O08528_C909")

inner_tidy = tibble(timepoint = tmp$timepoint, concentration = tmp$iso_over_control);
fit = lm(concentration ~ bs(timepoint, degree = 3), data = inner_tidy);
output = paste(predict(fit, data.frame(timepoint = seq_range)), collapse = "_");

pf(tmp_sum$fstatistic[1], tmp_sum$fstatistic[2], tmp_sum$fstatistic[3], lower.tail = F)
anova(fit)$`Pr(>F)`[1]

tmp_sum = summary(fit)
tmp_sum$coefficients[,4]
tmp_sum$r.squared
######### testor ######################################################
...
png(paste("2-15-18 Fig3 Dynamics Plot-all-fix.png", sep = ""), width = 600, height = 800);  
pdf(paste("2-15-18 Fig3 Dynamics Plot-all-fix.pdf", sep = ""), width = 6, height = 8);  
...
multiplot(dynamic_plot[[1]], dynamic_plot[[2]],
          ...
          #dynamic_plot[[7]], #dynamic_plot[[8]],
          # dynamic_plot[[9]], dynamic_plot[[10]],
          # dynamic_plot[[11]], dynamic_plot[[12]],
          # dynamic_plot[[13]], dynamic_plot[[14]],
          cols = 2);
dev.off();
...
dta_corr_result = 
  dta_corr_tmp %>% 
  #filter(cor_adjust_p_value < 0.05)
...
# positives %>% write_csv("p_tmp.csv")
# negatives%>% write_csv("n_tmp.csv")
...
positives = 
  positives %>% 
  separate(protein_ptm, into = c("UniProt Accession", "Cys Location"), sep = "_");
  
uniprot_positives =
  select(up, keys=c(positives$`UniProt Accession` %>% unique()), columns=c("ID", "GENES","PROTEIN-NAMES"),keytype="UNIPROTKB");

(positives$`UniProt Accession` %>% unique())

negatives

select(up, keys=c("P07724", "P31946","P62258", "P14211", "P19221"), columns=c("ID", "GENES","PROTEIN-NAMES"), keytype="UNIPROTKB")
...
###
po_tem = 
positives %>% select(protein_ptm) %>% unique() %>% separate(protein_ptm, into = c("protein", "location"), sep = "_")

ne_tem = 
negatives %>% select(protein_ptm) %>% unique() %>% separate(protein_ptm, into = c("protein", "location"), sep = "_") 

match(po_tem$protein, ne_tem$protein)


po_tem %>% write_csv("po_tem.csv")
ne_tem %>% write_csv("ne_tem.csv")
